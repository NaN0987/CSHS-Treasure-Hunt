...
class GifParticle {
  constructor() {
    ...
    this.size = Math.random() * 500 + 50; // Random size between 50px and 150px
    this.opacity = 1; // Initial opacity
    this.element = document.createElement("img"); // Use <img> element for GIFs
    this.element.src = `images/wini/win${Math.round(Math.random() * 4 + 1)}.gif`; // Replace with the path to your GIF
    this.element.style.position = "absolute";
    this.element.style.width = `${this.size+100}px`;
